...
class RegexUtil
{
    ...
    public static function isPhoneNumber(string $string) {
        return preg_match_all('/(\+)?([0-9]){10,20}/', $string);
    }
    ...
    public static function isAlphaNumaric(string $address) {
        return preg_match_all('/^[A-Za-z0-9\s]+$/', $address);
    }
    public static function isFloat(string $string) {
        return preg_match_all('/^([0-9]{1,9})(((\.)([0-9]{1,2})))?$/', $string);
    }

    public static function isPrice(string $string) {
        return preg_match_all('/^([0-9]{1,9})(((\.)([0-9]{1,2})))?$/', $string);
    }
}
